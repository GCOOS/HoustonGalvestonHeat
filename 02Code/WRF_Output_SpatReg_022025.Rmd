...
coeff <- t(apply( m.1$p.beta.samples, 2, quantile, c(.5, .025, .975) ))
se    <- apply( m.1$p.beta.samples, 2, sd )
coeff <- cbind( coeff[,1], se, coeff[,2:3])
print( signif(coeff, 3) )

write.csv(signif(coeff, 3), "SpDynLM_Coefficients_ParkBase2.csv")
saveRDS(m.1, "SyDynLm_mod.rds")
...
coeff <- t(apply( m.1$p.sigma.eta.samples, 2, quantile, c(.5, .025, .975) ))
se    <- apply( m.1$p.sigma.eta.samples, 2, sd )
coeff <- cbind( coeff[,1], se, coeff[,2:3])
print( signif(coeff, 3) )
