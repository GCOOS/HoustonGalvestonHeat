...
HourlyMeans17$LandUse <- LU_dict$New_Name[HourlyMeans16$LandUse]
...
ReOrg_Temp16 <- as.data.frame(matrix(0, ncol=(length(HoursOfInterest) + 2), nrow =n))
...
ReOrg_RH16 <- as.data.frame(matrix(0, ncol=(length(HoursOfInterest) + 2), nrow =n))
...
#columns are one for each hour, one for distance2ocean, and one for land use
ReOrg_Temp17 <- as.data.frame(matrix(0, ncol=(length(HoursOfInterest) + 2), nrow =n))
...
ReOrg_RH17 <- as.data.frame(matrix(0, ncol=(length(HoursOfInterest) + 2), nrow =n))
...
n16 <- length(unique(HourlyMeans16$GridCell)) #observations per year
p16 <- (length(unique(HourlyMeans16$LandUse))-1) + 1 #number of predictors, have to account for categorical land use

n17 <- length(unique(HourlyMeans17$GridCell)) #observations per year
p17 <- (length(unique(HourlyMeans17$LandUse))-1) + 1 #number of predictors, have to account for categorical land use
...
#run the model
m.1 <- spDynLM(mods, data=ReOrg_Temp16, coords=st_coordinates(coords_UTM),
